...
書く: 何を定めるかを冒頭で / なぜ必要になったか + 判断基準（decision drivers） /
決定と根拠をセットで（chosen-because） / 遵守の確認方法（confirmation） /
方針・原則・技術選定（ライセンス込み） / 理由つきの利点・トレードオフ / 見直し条件

書かない: 具体的な閾値（導出ルール + 一次ソースへのポインタのみ） / 実装詳細・手順書 /
陳腐化する列挙 / Issue・PR 番号 / 付随的な発見・先送り事項 / スケジュール / 作業ログ /
指標の目的化 / 疑似的な代替案（数合わせの候補） / 抽象度の混在した候補比較
